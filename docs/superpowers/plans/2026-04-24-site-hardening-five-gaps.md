...
1. Log in to `dash.cloudflare.com` as `***REMOVED-EMAIL***`
...
Expected: within 2-3 minutes, Cloudflare Workers Builds posts a comment on the PR containing a preview URL like `https://<hash>.haddrell-blog.***REMOVED-SLUG***.workers.dev`. Visit the URL — the site should render identically to production.
